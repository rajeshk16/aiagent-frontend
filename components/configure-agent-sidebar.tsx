...
export function ConfigureAgentSidebar() {
  ...
  const navigation = [
    {
      ...
      icon: Settings,
      current: pathname?.endsWith('/edit')
    },
    {
      name: "Additional Instructions",
      href: `/agents/${agentId}/edit/additional-instructions`,
      icon: FileText,
      current: pathname?.includes('additional-instructions')
    },
    {
      name: "Task Steps",
      href: `/agents/${agentId}/edit/task-steps`,
      icon: FileText,
      current: pathname?.includes('task-steps')
    }
  ]

  return (
    <div className="w-64 bg-white border-r flex flex-col fixed inset-y-0">
      <div className="p-6">
